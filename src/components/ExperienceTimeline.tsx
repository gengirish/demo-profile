    ...
    <section id="experience" className="section-padding" style={{ backgroundColor: "#0f0f23" }}>
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="mt-4" style={{ color: "#94a3b8" }}>14 years — from writing neural networks in 2012 to building multi-agent AI systems in 2024</p>
        </div>
            ...
            {[...experience].reverse().map((exp, i) => (
              <div key={i} className="relative pl-16">
                <div className="absolute w-5 h-5 rounded-full border-2 flex items-center justify-center"
                  style={{ left: "15px", top: "20px", borderColor: exp.domainColor, backgroundColor: `${exp.domainColor}22` }}>
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: exp.domainColor }} />
                </div>
                <div className="p-6 rounded-xl" style={{ background: "rgba(26,26,46,0.8)", border: "1px solid rgba(0,212,255,0.15)", backdropFilter: "blur(16px)" }}>
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                    ...
                    <div className="text-right">
                      ...
                      <span className="px-2 py-1 rounded-full text-xs mt-1 inline-block" style={{ color: exp.domainColor, backgroundColor: `${exp.domainColor}15`, border: `1px solid ${exp.domainColor}33` }}>{exp.phase}</span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2 mb-4">
                    {exp.highlights.map((h, j) => (
                      <li key={j} className="flex items-start gap-2 text-sm" style={{ color: "#cbd5e1" }}>
                        <span className="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: exp.domainColor }} />{h}
                      </li>
                    ))}
                  </ul>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {exp.tech.map(t => (
                      <span key={t} className="px-2 py-0.5 text-xs rounded border font-mono" style={{ color: "#94a3b8", backgroundColor: "#030014", borderColor: "#16213e" }}>{t}</span>
                    ))}
                  </div>
                  <div className="pt-4" style={{ borderTop: "1px solid #16213e" }}>
                    <div className="flex justify-between text-xs mb-1" style={{ color: "#64748b" }}>
                      <span>Model Accuracy</span>
                      <span className="font-mono" style={{ color: exp.domainColor }}>{exp.accuracy}%</span>
                    </div>
                    <div className="w-full rounded-full h-1.5" style={{ backgroundColor: "#030014" }}>
                      <div className="h-1.5 rounded-full" style={{ width: `${exp.accuracy}%`, backgroundColor: exp.domainColor }} />
                    </div>
                  </div>
                </div>
              </div>
